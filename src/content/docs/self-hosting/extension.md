...
viewer**, with no backend and no proxy in the byte path.
...
The extension is plain JS/CSS/JSON, with no bundler. You can load it unpacked:
...
The **same public listing** works on any self-hosted instance, so you don't need to fork
...
- Holds no secrets, since secret-bound sources stay on the backend.
