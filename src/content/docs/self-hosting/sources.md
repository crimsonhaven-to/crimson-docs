...
description: How to plug your own private sources engine into Crimson Haven, covering the public API contract the client expects and the CI wiring to bundle it.
---
...
This page documents the *mechanism*, meaning the contract your engine must satisfy and how
...
The client expects your engine at **`vendor/crimson-sources`** and imports its public
API through a Vite alias (`crimson-sources` → `vendor/crimson-sources/src/index.ts`).
Locally that directory is a git submodule; in CI it's a fresh clone at the branch tip
(see below). Either way the TypeScript is transpiled inline at build time, so there's
no separate build step for the engine.
...
The client supplies `signProxyUrl` and `resolveGrant` for you, and they call the
...
## Wiring your repo in for local development

Once your private repository implements the contract above, add it to the client as a
submodule so a local `npm run dev` picks it up:
...
- You can pin a branch (e.g. `dev` for staging, `main` for production) in
  `.gitmodules`.
- **The submodule pin is a local-development convenience only.** CI ignores it and
  clones the branch tip instead, so you never have to commit a submodule bump to ship
  an engine change.
...
longer bundles it and that fetch step is gone.)

Your sources repo's own CI can also fire the client's `workflow_dispatch` to rebuild
and redeploy on a `channel` of `dev` or `prod`, so pushing your engine ships it without
touching the client repo at all. See [The CI/CD pipeline](/deployment/cicd/).
...
So a fresh `git clone` of the client builds out of the box, with no stub to write and no
...
as **E0** sources that scrape and resolve *on the server*, using the very same
"named-by-a-secret" trick the client uses. The backend build reads two **Actions
secrets**:
...
:::caution[The wiring ships switched off]
Because this is the discouraged path, the overlay stage is **commented out** in the
backend's `Dockerfile` and in its `build-image.yml`. Setting the two secrets alone does
nothing. You have to uncomment the `private-sources` build stage, its `COPY` lines, and
the `secrets:` block in the workflow's build step first. That is deliberate: nobody
turns this on by accident.
:::

Once enabled and both secrets are set, the build clones that repo and drops its modules
into the backend's `scrapers/`, `resolvers/` and `manga/` packages; they're
auto-discovered and registered at boot. A runtime kill-switch,
`PRIVATE_SOURCES_ENABLED=0`, disables them without a rebuild. Like the client, the
**same public backend** builds cleanly for everyone, and a fork with no secret simply
gets the base image.

:::danger[Lumi says: think twice]
E0 puts the scraping back **on your server**, the exact thing the E1 to E3 split exists to
