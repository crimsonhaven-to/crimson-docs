...
description: How Crimson Haven's manga reading surface works, covering AniList discovery on the backend, MangaDex resolved in the viewer's browser, the reader, and the optional server-side provider.
---
...
surfaces, with the same login wall, the same favorites and the same "continue where you
left off", except the unit is a *chapter of pages*, not a stream.
...
The reading surface is purely additive. Anime, shows and movies are untouched, and manga
...
| --- | --- | --- |
| **Discovery + metadata** (trending, search, covers, synopsis) | **Backend** (AniList) | Public metadata, from the same place the anime surface already draws from. Names no manga host. |
...
MangaDex's API is **free and key-less**, so you never need an API key. Its JSON API does
*not* answer CORS for a third-party origin, so the three lookups (search → chapter feed
→ page server) need a CORS bypass, which is a "plain-CORS discovery" source in New-System terms
...
using the same `SOURCES_REPO` / `SOURCES_PAT` secrets. If your private sources repository
...
The same caveat applies: the overlay stage is **commented out** in the backend's
`Dockerfile` and workflow, so you must uncomment it before either secret does anything.
...
- Reading is a fourth, additive surface, and anime/shows/movies are untouched.
