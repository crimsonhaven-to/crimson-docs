...
1. **The client asks the backend for metadata** (`/info`, `/seasons`, …) so it knows
   the title, season and episode, drawn from TMDB + AniList and served from PostgreSQL.
2. **The client starts the backend's `/watch` stream.** The backend returns only the
   sources *it* owns (your Local / Cache / Jellyfin, if configured), which is usually
