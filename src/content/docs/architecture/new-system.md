...
A browser is a hostile place to fetch streams from gated CDNs, for five reasons:
...
A traditional backend dodges C1 through C4 by doing everything server-side. But that
puts **all video bytes** on the backend, the exact cost we want to avoid.
...
- **E0, the backend.** The floor. Serves operator-owned sources (Local / Cache /
...
- **E1, a plain browser fetch.** Real Chrome fingerprint and residential IP, but
...
- **`/sign`** mints a signed link to the edge proxy (E2). The proxy secret never
  reaches the browser.
- **`/resolve`** covers a secret-bound source: the backend does the secret part
...
to reading: discovery and metadata stay on the backend (AniList is public metadata, not
a manga host), while the chapter list and page images are resolved in the viewer's browser
...
private provider for devices that can run neither the extension nor the proxy, which is
exactly the same shape as everything above. See
