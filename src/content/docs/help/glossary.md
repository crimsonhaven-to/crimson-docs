...
Terms that recur throughout these docs.
...
| **E1: Plain browser fetch** | A real browser, but bound by CORS and forbidden headers. |
| **E2: Edge proxy** | Header injection + CORS relay, but from a datacenter IP. |
| **E3: Extension** | A real browser on the viewer's IP with header rewrite + CORS bypass. |

### Grant
A small, login-gated backend endpoint (`/scrape-meta`, `/sign`, `/resolve`) that gives
...
### Sources engine
Your private TypeScript package (bundled into the client) that finds and resolves
streams. The public stack ships none. See
[Adding your own sources](/self-hosting/sources/).

### NDJSON
Newline-delimited JSON: one JSON object per line. The `/watch` endpoint streams results
this way so each resolved source reaches the player as soon as it's ready.

### StreamLine
One line of the `/watch` NDJSON: `{type:"stream", source, streamType, url, …}`. The
client's engine emits the **same** shape, so local and backend streams merge cleanly.
...
on the user's device. No mail server needed; no recovery if the phrase is lost.
...
so even that source's bytes bypass the backend, without the secret ever reaching the
browser.
...
behind the [Live TV surface](/self-hosting/live-tv/). The backend fetches its JSON API
twice daily into an in-memory catalogue (no key, no database table) and honours the
project's blocklist.

### The Lumi grant
The per-account permission to talk to [Lumi's chatbot](/self-hosting/lumi/). Denied by
default for everybody, granted one member at a time on **Admin › Users**, and audited
like an admin promotion, because it lets that member spend money against your provider
key.
...
open) and answers from the result. Each is a thin wrapper over an existing engine, so
she can't contradict the rest of the API. A **tool round** is one such request and
answer, capped per reply so a model that keeps searching can't keep billing.

### Follow (subscription)
A member marking one anime so it shows on the
...
caller fetches, the rest await the same result. Applied per process to the AniList and
TMDB fetchers with the widest fan-in.

### Direct-first playback
The Live TV playback model: a feed the browser is *allowed* to load (https, no gated
headers) plays straight off the broadcaster's CDN with zero backend bandwidth. Only when
the browser's own rules (mixed content, CORS, gated headers) forbid the direct path does
it fall back to the signed `/iptv_proxy`.
