...
- **E0, the backend.** The floor; serves operator-owned sources and holds secrets.
- **E1, a plain browser fetch.** Real browser, but bound by CORS + forbidden headers.
- **E2, the edge proxy.** Header injection + CORS relay, but a datacenter IP.
- **E3, the extension.** A real browser on the viewer's IP with header rewrite + CORS bypass.
...
### Submodule
A git repository nested inside another. Locally, the client pulls your **sources** in as
a submodule; CI clones the same repo at its branch tip instead. (The companion extension
is no longer vendored, since it ships on the Chrome Web Store.) See
...
headers) plays straight off the broadcaster's CDN, using zero backend bandwidth, and only
