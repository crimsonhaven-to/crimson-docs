...
description: Enable the three media sources the backend can serve directly, namely Local files, the server-side Cache, and your own Jellyfin server.
---
...
1. Make the media readable by the backend container by bind-mounting your directory:
   ```yaml
...
Registering a Local source also lights up the
**[local media library](/self-hosting/local-library/)**: a browsable, searchable
*Index view* of everything on those roots (with per-title pages and watch progress),
not just the on-demand title match above.
:::
